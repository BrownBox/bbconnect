...
function envoyconnect_inbounds(){
    ...
    <div class="wrap">
        ...
        <h2><?php _e( 'Inbound Activity', 'envoyconnect' ); ?></h2>
...
class PP_Inbound_Table extends WP_List_Table {
	...
	function envoyconnect_inbound_query() {
		$pp_inbound = new WP_Query( array( 
											'post_type' => apply_filters( 'envoyconnect_inbound_posts', array( 'pp_log', 'pp_interaction' ) ), 
											'post_status' => array( 'publish', 'private' ), 
											'posts_per_page' => apply_filters( 'envoyconnect_inbound_per_page', -1 ), 
											'meta_query' => array(
													array(
														'key' => '_pp_log_type',
														'value' => 'import',
		...
		if ( $pp_inbound->have_posts() ) : while( $pp_inbound->have_posts() ) : $pp_inbound->the_post();
			...
			// GET THE TYPE
			$type = apply_filters( 'envoyconnect_inbound_type', '' );
			$type_label = apply_filters( 'envoyconnect_inbound_type_label', '' );
			if ( 'pp_log' == $post->post_type || 'pp_interaction' == $post->post_type ) {
				$types = array_merge( envoyconnect_user_log_types(), envoyconnect_user_interaction_types() );
				$type = get_post_meta( $post->ID, '_' . $post->post_type . '_type', true );
				if ( !isset( $types[$type] ) ) {
					continue;
				} else {
			...
			// GET THE STATUS
			//$status = get_post_meta( $post->ID, '_pp_action_status', true );
			//if ( 'archived' == $status ) continue;
    ...
    function __construct(){
        ...
        parent::__construct( array(
            'singular'  => 'envoyconnect_inbound', 
            'plural'    => 'envoyconnect_inbounds', 
            'ajax'      => true 
        ) );
    ...
    function column_name($item){
        $actions = array();
        $actions['edit'] = sprintf( '<a href="%s?page=envoyconnect_edit_user&user_id=%s">edit</a>', admin_url('users.php'), $item['uid'] ); 
    ...
    function column_type($item){
        //return sprintf( '<a class="envoyconnect-icon %1$s">&nbsp;</a> %2$s', $item['type'], $item['type_label'] );
    ...
    function process_bulk_action() {
        ...
        if( 'archive' === $this->current_action() ) {
        	//print_r($_REQUEST);
        	$postids = $_REQUEST['envoyconnect_inbound'];
    ...
    function prepare_items() {
         ...
         */
        $per_page = apply_filters( 'envoyconnect_inbound_per_page_paginate', 25 );
         ...
         */
        $data = $this->envoyconnect_inbound_query();
