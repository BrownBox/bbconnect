...
function add_new_search_row(){
	...
	jQuery.post( 
    	envoyconnectAdminAjax.ajaxurl, 
    	{ action : 'envoyconnect_new_search_row', data : tar, envoyconnect_admin_nonce : envoyconnectAdminAjax.envoyconnect_admin_nonce },
        function( response ) {
...
function select_search_form() { 
	...
	jQuery('#'+key+'-child').empty().html('<img src="'+envoyconnectSearchAjax.ajaxload+'" />');
	 
	jQuery.post( 
    	envoyconnectSearchAjax.ajaxurl, 
    	{ action : 'envoyconnect_search_form', fid : fid, key : key, envoyconnect_search_nonce : envoyconnectSearchAjax.envoyconnect_search_nonce },
        function( response ) {
            // DISPLAY THE RESPONSE
...
// REPORT UI TOGGLES
function envoyconnect_report_ui_action(){
...
function save_report() { 
	...
	//var title = jQuery('#envoyconnect-query-title').val();
	//var query = jQuery('#envoyconnect-query-transient').val();
	...
	// EMPTY OUT THE MAIN HOLDER'S ELEMENTS AND PLAY THE LOADER
	//jQuery('#save-query-form').empty().html('<img src="'+envoyconnectReportAjax.ajaxload+'" />');
	
	jQuery.post( 
    	envoyconnectAjax.ajaxurl, 
    	{ action : 'envoyconnect_process_queries', data : str },
        function( response ) {
            ...
            // EMPTY OUT THE MAIN HOLDER'S ELEMENTS AND PLAY THE LOADER
            jQuery('#reports-queries-display').empty().html('<img src="'+envoyconnectReportAjax.ajaxload+'" />');
             
            jQuery.post( 
            	envoyconnectAjax.ajaxurl, 
            	{ action : 'envoyconnect_process_queries', data : 'hi' },
                function( response ) {
...
function process_user_imports() { 
	...
	// EMPTY OUT THE MAIN HOLDER'S ELEMENTS AND PLAY THE LOADER
	jQuery('#reports-user-imports-display').empty().html('<img src="'+envoyconnectReportAjax.ajaxload+'" />');
	 
	jQuery.post( 
    	envoyconnectAjax.ajaxurl, 
    	{ action : 'envoyconnect_process_saved_imports', data : str },
        function( response ) {
...
function setListener(){
	jQuery.ajax({
		type : 'post',
		dataType : 'html',
		url : envoyconnectPollAjax.ajaxurl,
		data : {
		action: 'envoyconnect_poll_status',
		envoyconnect_poll_nonce : envoyconnectPollAjax.envoyconnect_poll_nonce
		},
...
function envoyconnect_report_submit() { 
    ...
    // EMPTY OUT THE MAIN HOLDER'S ELEMENTS AND PLAY THE LOADER
	jQuery('#report-display').empty().html('<div id="loader"><img src="'+envoyconnectReportAjax.ajaxload+'" /></div><div id="progress"></div>');
	//jQuery('#report-display').innerHTML = '';
	//jQuery('#report-display').html('<div id="loader"><img src="'+envoyconnectReportAjax.ajaxload+'" /></div><div id="progress"></div>');
    if ( jQuery(this).hasClass('import-process') ) {
    	...
    	jQuery.post( 
    		envoyconnectReportAjax.ajaxurl, 
    		{ 
    			action : 'envoyconnect_report_form', 
    			data : str, 
    			envoyconnect_report_nonce : envoyconnectReportAjax.envoyconnect_report_nonce 
    		}
    	);
        ...
        jQuery.ajax({ 
    		type: 'POST', 
	    	url: envoyconnectReportAjax.ajaxurl, 
	    	data: { 
	    		action : 'envoyconnect_report_form', 
	    		data : str, 
	    		envoyconnect_report_nonce : envoyconnectReportAjax.envoyconnect_report_nonce 
	        ...
	        success: function( response ) {
	            ...
	            jQuery('.report-go-go').click(envoyconnect_report_submit);
...
jQuery(document).ready(function () {
	...
	jQuery('#envoyconnect').on('click', '.query-add', add_new_search_row);
	jQuery('#envoyconnect').on('click', '.query-sub', sub_search_row);
	
	// SELECT ALL USERS OR ACTIONS TO EDIT OR ACT UPON
	jQuery('#report-nav').on('click', '.nav-tab', envoyconnect_report_ui_action);
	jQuery('#envoyconnect').on('change', '#master-gredit', get_user_gredits);
	jQuery('#envoyconnect').on('change', '#action-gredit', get_action_gredits);
	
	// MAIN AJAX SUBMISSION FORM
	jQuery('#envoyconnect').on('click', '.report-go', envoyconnect_report_submit);
	
	// PROCESS SAVED QUERIES
	jQuery('#envoyconnect').on('click', '#save-current-query', save_report);
	jQuery('#envoyconnect').on('click', '#process-reports-queries', process_save_report);
	
	// PROCESS SAVED QUERIES
	jQuery('#envoyconnect').on('click', '#process-reports-user-imports', process_user_imports);
	
	// INTERFACE TOGGLES
	jQuery('#envoyconnect').on('change', '#user-match-select', function(){
		if ( jQuery(this).val() != '' ) {
	...
	/*
	jQuery('#envoyconnect').on('click', '#filter', function(){
		if ( jQuery('#envoyconnect-query-title').attr('value').length != 0 ) {
