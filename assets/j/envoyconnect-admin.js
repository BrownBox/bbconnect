...
function profile_fields() { 
    ...
    jQuery('body').prepend('<div id="envoyconnect-overlay"></div>');
	jQuery('#envoyconnect-overlay').prepend('<div id="envoyconnect-modal">'+envoyconnectAdminAjax.oneMoment+'</div>');
    
    // SUBMIT THE FORM DATA
    jQuery.post( 
    	
    	envoyconnectAdminAjax.ajaxurl, { 
    		action : 'envoyconnect_profile_fields_update', 
    		data : str, 
    		envoyconnect_admin_nonce : envoyconnectAdminAjax.envoyconnect_admin_nonce 
    	},
        function( response ) {
            // DISPLAY THE RESPONSE
            jQuery('#envoyconnect-modal').html(response);
            //tb_show(null,'#TB_inline?height=500&width=630&inlineId=envoyconnect-ajax-loading&modal=true',null);
            setTimeout(function() {
            	jQuery('#envoyconnect-modal').fadeOut('slow');
            	//tb_remove();
            	window.location = '';
            },  3000);
        }
...
function new_meta_assign(){
	...
	if ( jQuery('#'+metakey+'-ele').length ) {
		...
		// LET THE USER KNOW
		if ( !jQuery('#message-'+conkey+' .envoyconnect-mk-error').length ) {
			jQuery('#message-'+conkey).append('<span class="envoyconnect-mk-error required example-text">'+envoyconnectAdminAjax.metaKeyTaken+'</span>');
		...
		if ( jQuery('#message-'+conkey+' .envoyconnect-mk-error').length ) {
			jQuery('#message-'+conkey+' .envoyconnect-mk-error').remove();
		...
		jQuery('#t-'+conkey+' [name*="envoyconnect_user_meta_options"]').each(function(i) {
			var narr = jQuery(this).attr('name'); 
			var newarr = narr.replace('['+oldkey+']','['+metakey+']');
			jQuery(this).attr('name',newarr);
		});
		jQuery('#t-'+conkey+' [name*="envoyconnect_user_taxonomy_options"]').each(function(i) {
			var narr = jQuery(this).attr('name'); 
			var newarr = narr.replace('['+oldkey+']','['+metakey+']');
			jQuery(this).attr('name',newarr);
		});
...
function remove_meta(){
	...
	jQuery(this).closest('.t-wrapper').find('[name*="envoyconnect_user_meta_options"]').each(function(i) {
		var narr = jQuery(this).attr('name'); 
		var newarr = narr.replace('['+rvar+']','[_paudel_'+rvar+']');
		jQuery(this).attr('name',newarr);
	});
	jQuery(this).closest('.t-wrapper').find('[name*="envoyconnect_user_taxonomy_options"]').each(function(i) {
		var narr = jQuery(this).attr('name'); 
		var newarr = narr.replace('['+rvar+']','[_paudel_'+rvar+']');
		jQuery(this).attr('name',newarr);
	});
...
function field_selection(){
	...
	// EMPTY OUT THE MAIN HOLDER'S ELEMENTS AND PLAY THE LOADER
	jQuery('#'+primary).empty().html('<img src="'+envoyconnectElemChoicesAjax.ajaxload+'" />');
	 
	jQuery.post( 
		envoyconnectElemChoicesAjax.ajaxurl, 
		{ action : 'envoyconnect_element_choices_form', meta_key : meta_key, field_type : field_type, envoyconnect_element_choices_nonce : envoyconnectElemChoicesAjax.envoyconnect_element_choices_nonce },
	    function( response ) {
	        // DISPLAY THE RESPONSE
...
function delete_form() {
	...
	var answer = confirm( envoyconnectAdminAjax.confirmDelete );
	if (answer) {
		jQuery.post( 
			envoyconnectAdminAjax.ajaxurl, { 
				action : 'envoyconnect_delete_form', 
				data : cid, 
		    	envoyconnect_admin_nonce : envoyconnectAdminAjax.envoyconnect_admin_nonce 
			},
		    function( response ) {
...
function check_profile(event) {
	...
	if ( jQuery('.halt').length != 0 ) {
		...
		if ( !jQuery('#errors').length ) {
			jQuery('#user-form').prepend('<div class="errors">'+envoyconnectAdminAjax.errMsg+'</div>');
		}
...
function envoyconnect_get_post_to_edit() {
	...
	// EMPTY OUT THE MAIN HOLDER'S ELEMENTS AND PLAY THE LOADER
	jQuery('#'+fid+' .envoyconnect-viewer').empty().html('<img src="'+envoyconnectAdminAjax.ajaxload+'" />');
	...
	jQuery.post( 
        ...
        function( response ) {
            ...
            //quicktags({id : cid});
            if ( !envoyconnectAdminAjax.firefox ) {
...
function envoyconnect_save_new_post() {
	...
	// EMPTY OUT THE MAIN HOLDER'S ELEMENTS AND PLAY THE LOADER
	jQuery('#'+sid+' div.envoyconnect-viewer').empty().html('<img src="'+envoyconnectAdminAjax.ajaxload+'" />');
...
jQuery(document).ready(function () {
			...
			update: function(event, ui) { 
				...
				if ( jQuery(ui.item).closest('ul').hasClass('section-list') ) {
					...
					if ( !jQuery('#'+sid+'-'+uid).length ) {
						...
						jQuery(ui.item).find('.section-input').val(tsid).after('<input id="'+sid+'-'+uid+'" class="section-output" type="hidden" name="envoyconnect_user_meta_options['+tsid+'][options][choices][]" value="'+tuid+'" />');
					}
				}
	...
	// DO IT...
	jQuery('#envoyconnect').on('click', '.rui', rui_toggle);
	...
	jQuery('#tim').click(function(){
		...
		jQuery.post( 
	    	envoyconnectAdminAjax.ajaxurl, 
	    	{ action : 'envoyconnect_new_elements_form', data : data, envoyconnect_admin_nonce : envoyconnectAdminAjax.envoyconnect_admin_nonce },
	        function( response ) {
