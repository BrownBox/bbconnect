...
function bbconnect_get_post_to_edit() {
	...
	jQuery('#'+fid+' .bbconnect-viewer').empty().html('<img src="'+bbconnectAjax.ajaxload+'" />');
	...
	jQuery.post( 
    	...
    	cid : cid, 
    	type : type, 
    	bbconnect_nonce : bbconnectAjax.bbconnect_nonce 
    	},
        function( response ) {
...
function bbconnect_save_new_post() {
	...
	// EMPTY OUT THE MAIN HOLDER'S ELEMENTS AND PLAY THE LOADER
	jQuery('#'+sid+' div.bbconnect-viewer').empty().html('<img src="'+bbconnectAjax.ajaxload+'" />');
