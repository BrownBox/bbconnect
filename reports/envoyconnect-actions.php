...
function envoyconnect_reports_actions( $display = '' ) {
?>
	<div id="group-action" class="drawer"<?php if ( 'open' == $display ) { echo ' style="display: block;"'; } ?>>
		
		<select id="action-select">
			<option value=""><?php _e( 'Select an Action', 'envoyconnect' ); ?></option>
			<optgroup label="<?php _e( 'Actions', 'envoyconnect' ); ?>">
				<option value="pp_export"><?php _e( 'Export', 'envoyconnect' ); ?></option>
				<?php do_action( 'envoyconnect_call_select_actions' ); ?>
			</optgroup>
		</select>
		...
		<div id="action-form">
			<?php 	
				pp_export();
				do_action( 'envoyconnect_show_selected_actions' );
			?>
		</div>
...
?>						
	<div id="pp_export" class="action-holder">
		<form id="pp_export_form" class="report-form" enctype="multipart/form-data" action="<?php echo admin_url( 'users.php?page=envoyconnect_reports' ); ?>" method="POST">
			<div class="inside">
				<div class="column_holder">
				
					<h3><?php _e( 'New Export', 'envoyconnect' ); ?></h3>
					<p><?php _e( 'What would you like to call this export?', 'envoyconnect' ); ?><br />
					<input type="text" name="export_title" class="regular-text" value="" /></p>
					
					<h3><?php _e( 'User Options', 'envoyconnect' ); ?></h3>
					<p><input type="checkbox" name="_grexport[all_users]" value="1" /> <?php _e( 'Select all users for this query', 'envoyconnect' ); ?></p>
					<p><input type="checkbox" name="_grexport[all_fields]" value="1" /> <?php _e( 'Select all possible fields', 'envoyconnect' ); ?></p>
					<?php do_action( 'envoyconnect_expo_user_opts' ); ?>
					
					<h3><?php _e( 'Actions Options', 'envoyconnect' ); ?></h3>
					<?php do_action( 'envoyconnect_expo_acts_opts' ); ?>
					
					<div class="tright">
						<input type="submit" name="export_members" value="<?php _e( 'Export', 'envoyconnect' ); ?>" class="button-primary report-go grexappeal" />
					</div>
					<input type="hidden" name="action[envoyconnect_export_process]" value="" />
...
function envoyconnect_export_process() {
	...
	if ( false != $users ) {
		...
		if ( isset( $_POST['_grexport']['all_fields'] ) ) {
			...
			$headers_prep = envoyconnect_get_user_metadata();
			foreach ( $headers_prep as $header ) {
				...
				if ( 'taxonomy' == $user_meta['options']['field_type'] ) {
					...
					if ( 'wpr' == $user_meta['source'] ) {
						...
						$headers['envoyconnect_'.$user_meta['meta_key']] = $user_meta['meta_key'];
					}
				}
			}
		...
		if ( isset( $_POST['gredit_actions'] ) )
			$exp_array['action_array'] = apply_filters( 'envoyconnect_export_action_array', $_POST['gredit_actions'] );
		
		foreach ( $users as $user_id ) {
			
			// ADD THE CURRENT USER IN
			$exp_array['user_id'] = $user_id;
			$cur_user = envoyconnect_rows( $exp_array );
			if ( isset( $cur_user['action'] ) ) {
				if ( isset( $_POST['_grexport']['all_actions'] ) || isset( $_POST['gredit_actions'] ) ) {
					$curact_array = apply_filters( 'envoyconnect_export_action_return', $cur_user['action'] );
		...
		$inc_file = envoyconnect_export_filename( sanitize_title_with_underscores( $exp_title ) );
		$ret_file = envoyconnect_export_file( $inc_file, $ret_array );
		if ( false != $ret_file ) {
			$export_results = '<div id="progress"><a href="' . $ret_file . '">' . __( 'Your file', 'envoyconnect' ) . '</a></div>';
		} else {
			$export_results = __( 'I was not able to complete the export.', 'envoyconnect' );
		}
				
	} else {
		$export_results = __( 'Please make a selection', 'envoyconnect' );
	}
...
function envoyconnect_export_filename( $exp_name, $extension = true ) {
...
function envoyconnect_export_file( $exp_filename, $ret_array ) {
