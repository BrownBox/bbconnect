...
function envoyconnect_pagination( $ret_res, $display = false ) {
	...
	$return_html = '<div class="pagination">';
 	...
 	if ( false != $display ) {
     	...
     	$return_html .= sprintf( __( 'Page %1$s', 'envoyconnect' ), $page_num.' '.$t_of.' '.$ret_res['max_num_pages'] );
...
function envoyconnect_report_process() {

	// RUN A SECURITY CHECK
	if ( ! wp_verify_nonce( $_POST['envoyconnect_report_nonce'], 'envoyconnect-report-nonce' ) )
		die ( 'terribly sorry.' );
	...
	if ( false != $ret_res ) {
		...
		update_option( '_envoyconnect_' . $current_user->ID . '_current', $filter_query );

		// DISPLAY THE RESULTS
		envoyconnect_report_display( $ret_res );
		//echo '<div>'.memory_get_usage().'</div>';
	}
...
// THE MAIN FUNCTION...
function envoyconnect_search() {

	global $ret_res, $current_user, $filter_query;
	$last_query = get_option( '_envoyconnect_' . $current_user->ID . '_current' );
	...
	<div id="envoyconnect" class="wrap">
		...
		<h2><a href="<?php echo admin_url( 'users.php' ); ?>" class="envoyconnect-refresh"><?php _e( 'User Reports', 'envoyconnect' ); ?></a></h2>

		<h2 id="report-nav" class="nav-tab-wrapper">
			<a id="filter-tab" class="nav-tab<?php if ( '' == $action_panel ) echo ' nav-tab-active'; ?>" title="filter">
				<span class="envoyconnect-icon query">&nbsp;</span>
				<div class="envoyconnect-icon-text"><?php _e( 'Search', 'envoyconnect' ); ?></div>
				<div class="envoyconnect-icon-ghost">&nbsp;</div>
			</a>
			<a class="nav-tab<?php if ( '' != $action_panel ) echo ' nav-tab-active'; ?>" title="group-action">
				<div class="envoyconnect-icon action">&nbsp;</div>
				<div class="envoyconnect-icon-text"><?php _e( 'Actions', 'envoyconnect' ); ?></div>
				<div class="envoyconnect-icon-ghost">&nbsp;</div>
			</a>
			<a class="nav-tab" title="group-edit">
				<div class="envoyconnect-icon edit">&nbsp;</div>
				<div class="envoyconnect-icon-text"><?php _e( 'Edits', 'envoyconnect' ); ?></div>
				<div class="envoyconnect-icon-ghost">&nbsp;</div>
			</a>
			<a class="nav-tab" title="saved-queries">
				<div class="envoyconnect-icon bookmark">&nbsp;</div>
				<div class="envoyconnect-icon-text"><?php _e( 'Saved Searches', 'envoyconnect' ); ?></div>
				<div class="envoyconnect-icon-ghost">&nbsp;</div>
			</a>
		</h2>
		<div id="report-options"><div class="inside">
			<?php /*wp_nonce_field('usermeta-search-nonce');*/ ?>
			<?php envoyconnect_filter_profile( $filter_query ); ?>
			<?php envoyconnect_reports_actions( $action_panel ); ?>
			<?php envoyconnect_group_edit( $edit_panel ); ?>
			<?php envoyconnect_reports_queries( $query_panel ); ?>
		</div></div>
		...
		<div id="report-display">
			<?php
				if ( isset( $report_upload_members ) ) {
					envoyconnect_upload_members_process();

				} else if ( isset( $report_import_members ) ) {
					$ret_res = envoyconnect_import_members_process();
					envoyconnect_report_display( $ret_res );

				} else if ( isset( $_GET['query'] ) ) {
					$ret_res = envoyconnect_filter_process( $filter_query );
					envoyconnect_report_display( $ret_res );
					update_option( '_envoyconnect_' . $current_user->ID . '_current', $filter_query );

				} else if ( isset( $_GET['import'] ) ) {
					$transient = get_option( $_GET['import'] );
					$ret_res = envoyconnect_filter_process( $transient['data'] );
					envoyconnect_report_display( $ret_res );

				} else if ( isset( $_GET['reset'] ) ) {
					delete_option( '_envoyconnect_' . $current_user->ID . '_current' );

				} else {
					$last_query = get_option( '_envoyconnect_' . $current_user->ID . '_current' );
					if ( false != $last_query ) {
						$ret_res = envoyconnect_filter_process( $last_query );
						envoyconnect_report_display( $ret_res );
					}
...
function envoyconnect_report_display( $ret_res = array() ) {
	...
	global $envoyconnect_address_count;
	$envoyconnect_user_metas = envoyconnect_get_user_metadata( array( 'group_break' => true ) );
	$envoyconnect_address_count = 0;
	foreach ( $envoyconnect_user_metas as $pmkey => $pmval ) {
		if ( false != strstr( $pmval, 'address' ) ) {
			$envoyconnect_address_count++;
		}
	}
	...
	if ( is_admin() ) {
		...
		if ( isset( $envoyconnect_errors ) && !empty( $envoyconnect_errors ) ) {

			echo '<div class="error">';

			if ( is_array( $envoyconnect_errors ) ) {
				foreach ( $envoyconnect_errors as $error ) {
		...
		do_action( 'envoyconnect_report_notifications', $ret_res );
	...
	if ( is_admin() ) {
		$report_nav .= envoyconnect_pagination( $ret_res );
	} else {
		$report_nav .= envoyconnect_pagination( $ret_res, true );
	}

	// ALLOW OTHER PLUGINS TO ADD VIEWS AND DO STUFF
	if ( is_admin() ) {
		$report_nav .= pautheme_toggle_view( apply_filters( 'envoyconnect_dir_tabs', array( 'list' ) ) );
	}
	$report_nav .= '</span>';
	echo $report_nav;

	do_action( 'envoyconnect_report_navigation' );
	...
	echo '</div></div>';
			...
			// HANDLE ACTIONS HERE
			do_action( 'envoyconnect_process_action_search', $action_search, $ret_res );
			$action_array = apply_filters( 'envoyconnect_process_action_array', false, $ret_res );
		?>
		...
		<div id="report-data-array" style="display: none;">
			...
			<input name="_grexport[envoyconnect_address_count]" type="hidden" value="<?php echo rawurlencode( serialize( $envoyconnect_address_count ) ); ?>" />
		...
		<form id="report-data-form" action="" enctype="multipart/form-data" method="POST">
			...
			<div id="list" class="content-display" style="<?php if ( !isset( $_COOKIE['view'] ) || 'list' == $_COOKIE['view'] ) { echo ' display: block;'; } ?>">
				...
				if ( false != $all_search && false != $member_search ) {
					...
					if ( false != $action_search ) {
						if ( is_admin() ) {
							...
							$t_head .= ' /><span style="padding: 0 0 0 4%;">'. __( 'Actions', 'envoyconnect' ) .'</span></td></tr></table></th>';
						} else {
							$t_head .= '<th>'. __( 'Actions', 'envoyconnect' ) .'</th>';
						}
					}
				...
				<table class="wp-list-table widefat" cellspacing="5">
					<thead>
						<tr>
							...
							<?php } ?>
							<th><?php echo apply_filters( 'envoyconnect_reports_user_header', __( 'Users', 'envoyconnect' ) ); ?></th>
							<?php echo $t_head; ?>
						</tr>
					</thead>
					<tbody>
				<?php
					...
					do_action( 'envoyconnect_prep_results_display', $ret_res );

					$user_rows = '';
					// FOR THE REST... PAGINATE!
					foreach( $member_search as $user_id) {
				 		$user_rows .= envoyconnect_rows( array( 'table_body' => $table_body, 'user_id' => $user_id, 'action_search' => $action_search, 'action_array' => $action_array, 'envoyconnect_address_count' => $envoyconnect_address_count, 'post_vars' => $ret_res['post_vars'], 'tdw' => $tdw ) );
					}
					echo $user_rows;
				?>
					</tbody>
				...
				<?php
					...
					echo '<p>' . __( 'terribly sorry. i did not find anything.', 'envoyconnect' ) . '</p>';
				}
			...
			<?php if ( is_admin() ) do_action( 'envoyconnect_results_display', $ret_res ); ?>
...
<?php
	...
	unset( $action_search );
	unset( $action_array );
	unset( $envoyconnect_errors );
	unset( $envoyconnect_query );
	unset( $envoyconnect_user_metas );
	unset( $envoyconnect_address_count );
...
function envoyconnect_grex_input( $args = '' ) {
	$defaults = array(
					'u_key' => '',
					'g_key' => '',
					'g_val' => ''
...
function envoyconnect_rows( $args = null ) {
	...
	if ( false == $return ) {
		...
		// RETRIEVE THE AVATAR
		$user_avatar = apply_filters( 'envoyconnect_reports_user_avatar', get_avatar( $current_member->user_email, 32 ), $current_member );

		// RETRIEVE THE PROFILE FIELDS
		$user_fields = array(
								$current_member->first_name . ' ' . $current_member->last_name,
		);
		$user_fields = apply_filters( 'envoyconnect_reports_user_fields', $user_fields, $current_member );

		// RETRIEVE THE ACTIONS
		$user_actions = array();
		if ( is_admin() ) {
			$user_actions['edit'] = '<a href="' . admin_url( '/users.php?page=envoyconnect_edit_user&user_id='.$current_member->ID ) . '">' . __( 'edit' ) . '</a>';
			$user_actions['email'] = '<a href="mailto:' . $current_member->user_email . '">' . __( 'email' ) . '</a>';
		} else {
			$user_actions['view'] = '<a href="' . home_url( '/envoyconnect/?rel=viewprofile&amp;uid=' . $current_member->ID ) . '" class="envoyconnectpanels-toggle" title="profile-' . $current_member->ID . '">' . __( 'view', 'envoyconnect' ) . '</a>';
		}
		$user_actions = apply_filters( 'envoyconnect_reports_user_actions', $user_actions, $current_member );
		...
		$return_html .= '<td class="envoyconnect-column-default" width="'.$tdw.'%">';
		$return_html .= '<div class="username column-username">';

		// USER AVATAR
		$return_html .= '<div class="envoyconnect-reports-user-avatar">' . $user_avatar . '</div>';

		// USER INFO - OPEN
		$return_html .= '<div class="envoyconnect-reports-user-info">';

		// USER FIELDS
		$return_html .= '<div class="envoyconnect-reports-user-fields">';
		$return_html .= implode( '<br />', $user_fields );
		$return_html .= '</span>';
		$return_html .= '<br />';

		// USER ACTIONS
		$return_html .= '<div class="envoyconnect-reports-user-actions">';
		...
		if ( false != $return_def ) {
			...
			//$return_val['organization'] = $current_member->envoyconnect_organization;
		}
		...
		foreach ( $table_body as $key => $value ) {
			...
			if ( is_array( $value ) ) {
				...
				if ( !empty( $current_member->$key ) ) {
					foreach ( $current_member->$key as $subkey => $subvalue ) {
						if ( 'envoyconnect' == substr( $key, 0, 12 ) )
							$key = substr( $key, 13 );
					...
					$ret_arr = '';
					//envoyconnect_grex_input( array( 'u_key' => $current_member->ID, 'g_key' => $key, 'g_val' => $current_member->$key ) );
				}

				if ( false == $return ) {
					if ( !is_array( $ret_arr ) ) {
						$return_html .= '';
						//$return_html .= envoyconnect_grex_input( array( 'u_key' => $current_member->ID, 'g_key' => $key, 'g_val' => $current_member->$key ) );
					} else {
						$return_html .= implode( ', ', $ret_arr );
						//$return_html .= envoyconnect_grex_input( array( 'u_key' => $current_member->ID, 'g_key' => $key, 'g_val' => strip_tags( implode( '|', $ret_arr ) ) ) );
					}
				} else {
					if ( !is_array( $ret_arr ) ) {
			...
			// META
				...
				//if ( isset( $current_member->$key ) ) {
					if ( is_array( $current_member->$key ) ) {
						...
						if ( false == $return ) {
							...
							//$return_html .= envoyconnect_grex_input( array( 'u_key' => $current_member->ID, 'g_key' => $key, 'g_val' => implode( '|', $marray_out ) ) );
						} else {
						...
						if ( false != strstr( $key, 'address' ) ) {
							...
							if (
								...
								foreach ( $post_op_arr as $pkey => $pval ) {
									$cur_po_preval = 'envoyconnect_'.$pval;
							...
							for ( $i = 1; $i <= $envoyconnect_address_count; $i++ ) {
							...
							if ( false == $return ) {
								...
								if ( false !== strpos( $key, 'address_state' ) ) {
									...
									foreach ( $cur_address as $ck => $cv ) {
										$cur_address_filtered[$ck] = envoyconnect_state_lookdown( $cur_ite, $cv );
									}
								...
								//$return_html .= envoyconnect_grex_input( array( 'u_key' => $current_member->ID, 'g_key' => $cur_grex_key, 'g_val' => $cur_grex_val ) );
							} else {
								if ( false === $post_vars ) {
									if ( false !== strpos( $origkey, 'address_state' ) ) {
										$return_val[$origkey] = envoyconnect_state_lookdown( $origkey, $current_member->$origkey );
									} else {
									...
									foreach ( $cur_address as $ka => $va ) {
										...
										if ( false !== strpos( $ka, 'address_state' ) ) {
											$return_val[$ka] = envoyconnect_state_lookdown( $ka, $va );
										} else {
											$return_val[$ka] = $va;
										}
									}
				...
				/*
					...
					if ( false == $return ) {
						$return_html .= '';
						//$return_html .= envoyconnect_grex_input( array( 'u_key' => $current_member->ID, 'g_key' => $key, 'g_val' => 'null' ) );
					} else {
						$return_val[$key] = '';
					}
		...
		if ( isset( $action_array[$current_member->ID] ) ) {
			foreach ( $action_array[$current_member->ID] as $key => $value ) {
				...
				if ( false == $return ) {
					...
					$inner_return_hmtl = '';
					if ( is_admin() ) {
						$return_html .= apply_filters( 'envoyconnect_action_detail_html', $inner_return_hmtl, $value );
					} else {
						$return_html .= apply_filters( 'envoyconnect_action_detail_html_public', $inner_return_hmtl, $value );
					}
					...
					$return_val = apply_filters( 'envoyconnect_action_detail_val', $return_val, $value );
