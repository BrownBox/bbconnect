...
function envoyconnect_filter_process( $post_data ) {
		...
		if ( !isset( $post_data['all_search'] ) ) {
			...
			if ( isset( $post_data['search'] ) ) {
				...
				foreach ( $post_data['search'] as $key => $value ) {
					...
					if ( 'envoyconnect' != substr( $fkey, 0, 12 ) ) {
						$option_key = 'envoyconnect_' . $fkey;
					} else {
						$option_key = $fkey;
					}
					...
					if ( 'skip' != $op ) {
						...
						if ( isset( $value['post_ops'] ) ) {
							...
							foreach ( $value['post_ops'] as $post_op ) {
								...
								$mtjoin[$mtc] = "INNER JOIN $wpdb->usermeta " . $mtc_as . "ON ($wpdb->users.ID = " . $mtc_dot . "user_id)";
								// PREPARE $mtquery[$mtc] = "(" . $mtc_dot . "meta_key = 'envoyconnect_" . $post_op . "')";
								$mtquery[$mtc] = $wpdb->prepare( "(" . $mtc_dot . "meta_key = %s)", "envoyconnect_$post_op" );
						...
						$skip_arr[] = 'envoyconnect_'.$value['field'];
					}
					...
					if ( 'taxonomy' == $user_meta['options']['field_type'] ) {
						...
						if ( 'envoyconnect' != $user_meta['source'] ) {
							$switch_key = $fkey;
						} elseif ( 'envoyconnect' == $user_meta['source'] ) {
				...
				if ( isset( $ometa ) ) {
					...
					if ( in_array( $order_by, $skip_arr ) ) {
						...
						$t_order_by = envoyconnect_get_option( $order_by );
						if ( false != $t_order_by ) {
							if ( 'envoyconnect' == $t_order_by['source'] )
								$order_by = 'envoyconnect_'.$order_by;
						}
						...
						if ( isset( $t_order_by['group'] ) && false !== strpos( $t_order_by['group'], 'address' ) ) {
							...
							$nullquo = $wpdb->prepare( $mtc_dot . "meta_key LIKE %s", '%envoyconnect_'.$addapp.'%' );
						} else {
			...
			do_action( 'envoyconnect_search_extend', $post_data, $all_search );
...
function envoyconnect_query_builder( $key, $value ) {

	if ( is_array( $value ) ) {
		foreach ( $value as $subkey => $subvalue ) {
			if ( 'wp' == $envoyconnect_{$key}['source'] ) {
				...
				if ( 'address' == substr( $envoyconnect_{$key}['group'], 0, 7 ) ) {
		...
		if ( 'wp' == $envoyconnect_{$key}['source'] ) {
			...
			if ( isset( $envoyconnect_{$key}['group'] ) && 'address' == substr( $envoyconnect_{$key}['group'], 0, 7 ) ) {
