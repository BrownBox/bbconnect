...
function envoyconnect_get_field( $args = '' ) {
	...
	$defaults = array( 
					...
					'post_val' => false, 
					'envoyconnect_cap' => '', // NEEDED TO PASS THROUGH FOR GROUPS AND SECTIONS
					'required' => false, 
	...
	if ( false != $post_val )
		$post_val = envoyconnect_scrub( 'envoyconnect_sanitize', $post_val );
	...
	if ( 'option' === $type ) {
		...
		if ( 'textarea' == $meta['options']['field_type'] ) {
			$field['value'] = envoyconnect_scrub( 'envoyconnect_esc_html', get_option( $key ) );
		} else {
			$field['value'] = envoyconnect_scrub( 'envoyconnect_esc_attr', get_option( $key ) );
		}
		if ( false != $post_val )
		...
		if ( 'textarea' == $meta['options']['field_type'] ) {
			$field['value'] = envoyconnect_scrub( 'envoyconnect_esc_html', get_post_meta( $id, $key, true ) );
		} else {
			$field['value'] = envoyconnect_scrub( 'envoyconnect_esc_attr', get_post_meta( $id, $key, true ) );
		}
		if ( false != $post_val )
			$field['value'] = envoyconnect_scrub( 'envoyconnect_esc_attr', $post_val );
		$public = false;
		...
		$field_pre_name = 'envoyconnect_user_meta';
		...
		if ( false != $post_val ) {
			...
			if ( 'wpr' == $meta['source'] ) {  
				...
				$field['value'] = envoyconnect_scrub( 'envoyconnect_esc_attr', get_user_meta( $id, $key, true ) );
			} else {
				if ( 'textarea' == $meta['options']['field_type'] ) {
					$field['value'] = envoyconnect_scrub( 'envoyconnect_esc_html', get_user_meta( $id, 'envoyconnect_'.$key, true ) );
				} else {
					$field['value'] = envoyconnect_scrub( 'envoyconnect_esc_attr', get_user_meta( $id, 'envoyconnect_'.$key, true ) );
				}
			}
		...
		$user_opt = envoyconnect_scrub( 'esc_attr', get_user_meta( $id, 'envoyconnect_pp_public', true ) );
		if ( false != $public ) { 
			...
			// IF A USER HAS NOT DECIDED YET ON THEIR PUBLIC STATUS...
				...
				$admin_opt = get_option( '_envoyconnect_public' );	
				if ( 'true' != $admin_opt ) {
					$pmt = 'off';
					$pmv = 'false';
				} else {
	...
	if ( '-view' == $action || 'view' == $action ) {
		...
		$field['action'] = '-view';
		$label_wrap = false;
		if ( 'true' == get_option( 'envoyconnectpanels_public_labels' ) ) {
			$label = '<span class="envoyconnectpanels-profile-label">'.$label.'</span>';
		} else {
			$label = false;
		}			
		$help = false;
	...
	$field['capabilities'] = $envoyconnect_cap;
	
	if ( false != $required || !empty( $meta['options']['req'] ) ) {
		if ( 'envoyconnect' == $meta['source'] && false !== strpos( $meta['meta_key'], 'address_state_' ) ) {
	...
	if ( false != $required && 'checkbox' == $meta['options']['field_type'] && false == $label )
		$mayberequired = __( 'Required', 'envoyconnect' );
	...
	if ( $wrap ) echo '<li class="meta-item">';
		...
		if ( $label_wrap ) echo '<span class="envoyconnect-label'.$required.'">'.$mayberequired;
			...
			if ( $admin_only ) echo '<a class="icon-admin" title="' . __( 'Only visible to admins.', 'envoyconnect' ) . '">&nbsp;</a>';
			if ( $public ) echo '<a class="pmt '. $pmt .'" rel="' . __( 'Public or Private', 'envoyconnect' ) . '" title="public_'. $key .'"><input type="hidden" id="public_'. $key .'"  name="' . $field_pre_name . '[pp_public]['. $key .']" value="'. $pmv .'" />&nbsp;</a>';
		if ( $label_wrap ) echo '</span>';
		
		if ( $label_wrap ) echo '<span class="envoyconnect-field">';
		
			envoyconnect_field( $meta, $field );
		if ( $help && 'checkbox' == $meta['options']['field_type'] ) echo '<a class="help" title="' . $help . '">&nbsp;</a>';
		if ( $label_wrap ) echo '</span>';
...
function envoyconnect_field( $meta, $args = array() ) {

	// CORNERSHOP CREATIVE CUSTOMIZATION
	$meta = apply_filters ( 'envoyconnect_field_prepare', $meta, $args );
	...
	switch( $meta['options']['field_type'] ) :
		...
		case 'title' : // BUMP
			...
			echo '<h3 class="envoyconnect-section">' . stripslashes( $meta['name'] ) . '</h3>';
		...
		case 'taxonomy' :
			...
			if ( '-view' == $action ) {
				...
				$display = get_option( '_envoyconnect_taxonomy_display' );
				...
				// SPECIAL CASES	
				if ( '-bulk-edit' == $action )
					echo '<div style="clear: both; padding: 7px;"><input type="radio" name="envoyconnect_user_taxonomy_options[' . $key . ']" value="append" checked="checked" /> ' . __( 'Append', 'envoyconnect' ) . '<input type="radio" name="envoyconnect_user_taxonomy_options[' . $key . ']" value="overwrite" /> ' . __( 'Overwrite', 'envoyconnect' ) . '<input type="radio" name="envoyconnect_user_taxonomy_options[' . $key . ']" value="remove" /> ' . __( 'Remove', 'envoyconnect' ) . '</div>';
				
				if ( '-edit' == $action && current_user_can( 'list_users' ) )
					echo '<div class="end-of-float"><a href="' . get_bloginfo('wpurl') . '/wp-admin/edit-tags.php?taxonomy=' . $tax . '&TB_iframe=true&height=450&width=920" class="thickbox button-primary">' . __( 'Add terms', 'envoyconnect' ) . '</a></div>';
		...
		case 'section' :
			...
			if ( '-view' != $action || 'true' == get_option( 'envoyconnectpanels_public_labels' ) )
			...
			if ( !empty( $meta['options']['choices'] ) ) {
				...
				foreach ( $meta['options']['choices'] as $skey => $sval ) { 
					...
					// GET THE OPTION
					$smeta = envoyconnect_get_option( $sval );
					
					// CHECK TO SEE IF ANY FIELDS HAVE BEEN REMOVED FROM DISPLAY
					$grouped_hide = envoyconnect_hide_meta( array( 'meta' => $smeta, 'envoyconnect_cap' => $capabilities, 'action' => $action ) );
					if ( $grouped_hide )
						continue;
					...
					// ECHO OUT THE FIELDS!
					envoyconnect_get_field( array( 'meta' => $smeta, 'id' => $cid, 'action' => $action, 'type' => $type, 'post_val' => $post_val ) );
		...
		case 'multiselect' :
			...
			if ( '-view' == $action ) {
				...
				if ( '-search' == $action ) {
					...
					$multiple = ' multiple="multiple" data-placeholder="' . __( 'All Options', 'envoyconnect' ) . '" style="width: 150px"';
					$name = $name . '[]';
				...
				if ( 'wpr' == $meta['source'] && 'role' == $key ) {
					...
					echo '<select name="' . $name . '" id="' . $id . '" class="regular-text' . $class . $autocomp . '"' . $multiple . '>';
					...
					envoyconnect_dropdown_roles( $user_role );
				...
				// AND DISPLAY NAMES (ADDING A FEW OPTIONS ALONG THE WAY)
					...
					if ( is_user_logged_in() && !empty( $value ) ) {
						...
						// FOR ORGS
						if ( !empty($cuser->envoyconnect_organization) )
							$public_display['display_organization'] = $cuser->envoyconnect_organization;
							
						if ( !empty($cuser->first_name) && !empty($cuser->last_name) ) {
							
							// SETUP FORMAL VARS
							if ( !empty( $cuser->envoyconnect_prefix ) )
								$m_pre = $cuser->envoyconnect_prefix . ' ';
								
							if ( !empty( $cuser->envoyconnect_suffix ) )
								$m_suf = ' ' . $cuser->envoyconnect_suffix;
								
							if ( !empty( $cuser->envoyconnect_middle_name ) ) {
								$m_mid = ' ' . $cuser->envoyconnect_middle_name . ' ';
							} else {
								$m_mid = ' ';
							}
		...
		case 'checkbox' :
			...
			if ( '-bulk-edit' == $action ) {
				...
				echo '<option value="true">' . __( 'yes', 'envoyconnect' ) . '</option>';
				echo '<option value="false">' . __( 'no', 'envoyconnect' ) . '</option>';
				echo '</select>';
					
			} else if ( '-search' === $action ) {
			
				echo '<select name="' . $name . '">';
				echo '<option value=""' . selected( $value, '' ) . '>' . __( 'empty', 'envoyconnect' ) . '</option>';
				echo '<option value="true"' . selected( $value, 'true' ) . '>' . __( 'yes', 'envoyconnect' ) . '</option>';
				echo '<option value="false"' . selected( $value, 'false' ) . '>' . __( 'no', 'envoyconnect' ) . '</option>';
				echo '</select>';
		...
		case 'radio' :
			...
			if ( '-view' == $action ) {
				...
				foreach( $option_arr as $subkey => $subvalue ) {			
					...
					echo '<li><input type="radio" name="' . $name . '" value="' . $subkey . '"' .  $checked . ' /> ' .  $subvalue . '</li>';
		...
		case 'group' :
			...
			if ( '-view' == $action ) {
				...
				foreach ( $meta['options']['choices'] as $gkey => $gvalue ) { 
					...
					$grouped_hide = envoyconnect_hide_meta( array( 'meta' => $gmeta, 'envoyconnect_cap' => $capabilities, 'action' => $action ) );
					if ( $grouped_hide )
						continue;
					...
					envoyconnect_get_field( array( 'meta' => $gmeta, 'id' => $cid, 'action' => $action, 'type' => $type, 'post_val' => $post_val ) );
				}
				
			} else { 
			
				$group_hide = envoyconnect_hide_meta( array( 'meta' => $meta, 'group_override' => true, 'envoyconnect_cap' => $capabilities, 'action' => $action ) );
				if ( false == $group_hide ) {
					?>
					<li>
						<div class="t-wrapper group">
							<p class="t-trigger-wrapper group">
								<span class="envoyconnect-label group">
								...
								<?php
									...
									do_action( 'envoyconnect_group_markers', $meta, $cid, $action );
								?>
								</span>
							</p>
							<div id="<?php echo $id.$action; ?>" class="t-panel">
								...
								<?php 
									foreach ( $meta['options']['choices'] as $gkey => $gvalue ) { 
										...
										// GET THE OPTION
										$gmeta = envoyconnect_get_option( $gvalue );
				...
				*/
					...
					?>
						<li id="<?php echo $id .''. $fkey; ?>" class="multilist">
							...
							<span>
								<?php echo $add_fields; ?>
								<input type="text" name="<?php echo $name . '['.$nfkey.'][value]'; ?>" class="regular-text regular-multi<?php echo $class; ?>" style="width: <?php echo $fper; ?>%;" value="<?php echo $value[$fkey]['value']; ?>" placeholder="<?php _e( 'Value', 'envoyconnect' ); ?>" />
								<?php echo $key_type; ?>
							</span> 
							<?php if ( $arrcount == 1 ) { ?>
		...
		case 'password' :
			...
			if ( '-view' != $action ) {
				...
				if ( 'wpr' == $meta['source'] ) {
					?>
					<li><input type="password" autocomplete="off" name="<?php echo $key.$choices[0]; ?>" id="<?php echo $key.$choices[0].$action; ?>" class="regular-text<?php echo $class; ?>" value="" /> <?php _e( 'type password', 'envoyconnect' ); ?></li>
					<li><input type="password" autocomplete="off" name="<?php echo $key.$choices[1]; ?>" id="<?php echo $key.$choices[1].$action; ?>" class="regular-text<?php echo $class; ?>" value="" /> <?php _e( 're-type password', 'envoyconnect' ); ?></li>
					<?php
				} else {
					?>
					<li><input type="password" autocomplete="off" name="<?php echo $name; ?>[<?php echo $choices[0]; ?>]" id="<?php echo $key.$choices[0].$action; ?>" class="regular-text<?php echo $class; ?>" value="" /> <?php _e( 'type password', 'envoyconnect' ); ?></li>
					<li><input type="password" autocomplete="off" name="<?php echo $name; ?>[<?php echo $choices[1]; ?>]" id="<?php echo $key.$choices[1].$action; ?>" class="regular-text<?php echo $class; ?>" value="" /> <?php _e( 're-type password', 'envoyconnect' ); ?></li>
					<?php
				}
		...
		case 'date' :
			...
			echo '<input type="text" name="'.$name.'" id="'.$id.'" class="regular-text envoyconnect-date'.$class.'" value="'.$value.'" />';
		...
		case 'text' :
			...
			if ( '-view' == $action ) { 
				...
				if ( '-search' == $action && 'user_registered' == $key ) {
					...
					echo '<input type="text" name="'.$name.'[]" id="'.$id.'_start" placeholder="'.__( 'start', 'envoyconnect' ).'" class="regular-text envoyconnect-date'.$class.'" value="'.$vs.'" />';
					echo '<input type="text" name="'.$name.'[]" id="'.$id.'_end" placeholder="'.__( 'end', 'envoyconnect' ).'" class="regular-text envoyconnect-date'.$class.'" value="'.$ve.'" />';
					...
					echo $value . '<br />';
					if ( 'ID' == $key )
						echo implode( '<br />', apply_filters( 'envoyconnect_user_id_assets', array(), $value, $action ) );
		...
		default :
			do_action( 'envoyconnect_field_switch', $meta, $args );
		break;
