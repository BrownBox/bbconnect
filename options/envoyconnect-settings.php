 ...
 * @since 1.0.2
 *
 * @param none This should take no parameters but should call the general envoyconnect_options_form.
 *
 * @return html outputs the options page with the activated option tab + contents.
 */

function envoyconnect_options() {

	// THE NAVIGATION
	// THE KEYS SHOULD BE NAMED PLACEHOLDERS FOR THE FUNCTION THAT RETURNS AN ARRAY OF FIELD PARAMETERS
	$navigation = apply_filters( 'envoyconnect_options_tabs', array( 
		
		'envoyconnect_general_settings' => array( 
									'title' => __( 'General', 'envoyconnect' ), 
									'subs' => false, 
							),
		
		'envoyconnect_pro_settings' => array( 
									'title' => __( 'Pro', 'envoyconnect' ), 
									'subs' => false, 
							),
							
		'envoyconnect_user_settings' => array( 
									'title' => __( 'Users', 'envoyconnect' ), 
									'subs' => false, 
							),
		
		'envoyconnect_actions_settings' => array( 
									'title' => __( 'Actions', 'envoyconnect' ), 
									'subs' => false, 
							),
							
		'envoyconnect_system_settings' => array( 
									'title' => __( 'System', 'envoyconnect' ), 
									'subs' => false, 
							),
							
		'envoyconnect_forms_settings' => array( 
									'title' => __( 'Forms', 'envoyconnect' ), 
									'subs' => false, 
							),
									
		'envoyconnect_panel_settings_trans' => array( 
									'title' => __( 'Panels', 'envoyconnect' ), 
									'subs' => false, 
							),
	) );
	
	envoyconnect_options_form( __( 'EnvoyConnect Options', 'envoyconnect' ), $navigation );
...
function envoyconnect_options_save() {	
	...
	if ( isset( $_POST['_pp_option'] ) ) { 
		...
		// SANITIZE ALL INPUT
		envoyconnect_scrub( 'envoyconnect_sanitize', $_POST );
		
		// LET PLUGINS MANIPULATE THEIR OWN OPTIONS
		do_action( 'envoyconnect_options_save_ext' );
		
		global $notice, $envoyconnect_flush;
		$notice = array();
		$envoyconnect_flush = apply_filters( 'envoyconnect_flush_permalinks', false, $_POST );
...
function envoyconnect_options_form( $form_title, $tabs = array() ) {	
	...
	if ( !empty( $notice ) )
		echo '<p class="envoyconnect_notice">' . __( 'Settings Updated', 'envoyconnect' ) . ': ' . implode( ', ', $notice ) . '</p>';
...
?>
	...
	<div id="icon-options-general" class="icon32"><br /></div>
		...
		<form enctype="multipart/form-data" action="" method="POST">
			...
			<?php 
				// SET THE NONCE
				wp_nonce_field('envoyconnect-nonce');
				
				// DISPLAY ANY SUB NAVIGATION
				if ( !empty( $tabs[$active]['subs'] ) ) {
					$sub_nav = envoyconnect_options_sub_nav( $active, $tabs[$active]['title'], $tabs[$active]['subs'] );
					echo $sub_nav['nav'];
					$active = $sub_nav['active'];
				}
				
				// FLUSH THE PERMALINKS IF WE NEED TO DO ANY REWRITES
				global $envoyconnect_flush;
				if ( false != $envoyconnect_flush ) {
					flush_rewrite_rules();					
				}
				
				// DO ANY PRE OPTION KINDA THINGS
				do_action( 'envoyconnect_options_pre', $active );
			?>
						...
						<?php
							...
							if ( !empty( $options ) ) {
								foreach ( $options as $setting ) {
									...
									envoyconnect_get_field( $setting );
								}
							}
						?>
						</ul>
			...
			<?php
				...
				do_action( 'envoyconnect_options_post', $active );
			?>
			<p class="submit"><input type="submit" name="save_options" value="<?php _e( 'Save Your Options', 'envoyconnect ' ); ?>" class="button-primary" /></p>
...
function envoyconnect_options_sub_nav( $active, $active_title, $subs = array() ) {
	$sub_nav = array();
	$sublinks = array();
	if ( isset( $_GET['sub'] ) ) { 
		$subactive = $_GET['sub'];
...
add_action( 'wp_ajax_envoyconnect_ffc_new_option', 'envoyconnect_ffc_new_option' );
function envoyconnect_ffc_new_option() {
	$context = $_POST['context'];
	$v = $_POST['v'];
	do_action( 'envoyconnect_ffc_option', $context, $v, false );
	die();
}
function envoyconnect_ffc( $args = null ) {
	...
	$defaults = array( 
					...
					'options' => false, 
					'columns' => 1, 
					'title' => __( 'Add fields', 'envoyconnect' ), 
				);
...
?>
	...
	<div id="<?php echo $context; ?>_select" class="options-field options-selector">
		...
		<a id="<?php echo $context; ?>_add_field" class="button add-ffc-field"><?php _e( '+ Add', 'envoyconnect' ); ?></a>
	</div>
						...
						<div class="t-wrapper<?php echo $alt_error; ?>">
							...
							<div id="option-<?php echo $context.$v; ?>" class="inside t-panel" style="display: none;">
								...
								<div id="option-<?php echo $context.$v; ?>-field">
									<?php do_action( 'envoyconnect_ffc_option', $context, $v, $c_array ); ?>
								</div>
							</div>
						</div>
					</li>
		...
		jQuery(document).ready(function() {
			...
			jQuery('#<?php echo $context; ?>_add_field').click(function(){
				...
				jQuery('#option-<?php echo $context; ?>'+fid).each(function(){
					...
					// PLAY THE LOADER
					jQuery('#option-<?php echo $context; ?>'+fid+'-field').empty().html('<div style="padding:10px 0;"><img src="'+envoyconnectAdminAjax.ajaxload+'" /></div>');
						 
					jQuery.post( 
						envoyconnectAdminAjax.ajaxurl, 
						{ action : 'envoyconnect_ffc_new_option', v : fid, context : '<?php echo $context; ?>', envoyconnect_admin_nonce : envoyconnectAdminAjax.envoyconnect_admin_nonce },
					    function( response ) {
