...
/**
 * Receives PAUPRESS-specific data and prepares it for WP insertion. 
 * On insertion scenarios, WORDPRESS takes the lead. 
 * On update scenarios PAUPRESS takes the lead.
 *
 * @since 1.0.2
 *
 ...
 * @param str $log_type Optional. The type of PAUPRESS action to document the source of the insertion.
 * @param str $log_code Optional. The source code of the PAUPRESS action.
 * @param str $title Optional. The title of the PAUPRESS action.
 * @param str $content Optional. The content of the PAUPRESS action.
 ...
 */
function envoyconnect_insert_user( $args = '' ) {
	...
	*/
	...
	// SCRUB!
	$ivals = envoyconnect_scrub( 'envoyconnect_sanitize', $ivals );
	...
	if ( false != $update && false != $match ) {
		...
		if ( !in_array( $match, $wp_match_reserve ) ) {
			//$wpdb->flush();
			global $wpdb;
			$match_value = $wpdb->get_results( $wpdb->prepare( "SELECT $wpdb->usermeta.user_id FROM $wpdb->usermeta WHERE $wpdb->usermeta.meta_value = %s", $ivals['envoyconnect_user_meta'][$match] ), ARRAY_N );
			$wpdb->flush();
	...
	if ( !empty( $ivals['user_login'] ) ) {
		...
		$username_prefix = get_option( '_envoyconnect_username_prefix' );
		$upre = '';
		...
		$userdata['user_login'] = envoyconnect_random( array( 'name' => $upre, 'compact' => true ) );
	}
	...
	*/
	...
	if ( !empty( $ivals['display_name'] ) ) {
		...
		$dname = '';
		if ( isset( $ivals['envoyconnect_user_meta']['first_name'] ) || isset( $ivals['envoyconnect_user_meta']['last_name'] ) ) {
			if ( isset( $ivals['envoyconnect_user_meta']['first_name'] ) )
				$dname .= $ivals['envoyconnect_user_meta']['first_name'] . ' ';
				
			if ( isset( $ivals['envoyconnect_user_meta']['last_name'] ) )
				$dname .= $ivals['envoyconnect_user_meta']['last_name'];
				
		} else if ( isset( $ivals['envoyconnect_user_meta']['organization'] ) ) {
			$dname .= $ivals['envoyconnect_user_meta']['organization'];
		}
		$userdata['display_name'] = trim( $dname );
	}

	// SET THE NICKNAME
	if ( !empty( $ivals['envoyconnect_user_meta']['nickname'] ) ) {
		$userdata['nickname'] = $ivals['envoyconnect_user_meta']['nickname'];
	} else if ( !$user ) {
		if ( isset( $ivals['envoyconnect_user_meta']['first_name'] ) ) {
			$fname = $ivals['envoyconnect_user_meta']['first_name'];
		} else {
			$fname = '';
		}
	...
	if ( !empty( $ivals['show_admin_bar_front'] ) ) {
		...
		$sabf = envoyconnect_get_option( 'show_admin_bar_front' );
	...
	envoyconnect_update_user_metadata( array( 'user_id' => $user_id, 'uvals' => $ivals, 'data_handler' => $data_handler ) );
...
/**
 * Receives PAUPRESS-specific data and prepares it for WP updates. On update scenarios, PAUPRESS takes the lead.
 *
 * @since 1.0.2
 *
 ...
 * @return int/arr The ID if insertion was successful, otherwise a WP_Error.
 */
function envoyconnect_update_user_metadata( $args = '' ) {
	...
	// IN ALL CASES EXCEPT NEW USERS, PAUPRESS LEADS
	if ( false != $source ) {
		do_action( 'envoyconnect_trigger_user_metadata', $user_id, $uvals, $source );
	}
		
	// SANITIZE THE DATA
	$uvals = envoyconnect_scrub( 'envoyconnect_sanitize', $uvals );
	
	// PROCESS THE USER META / MERGE TAGS
	// IF LOGIC PREVENTS NASTY NOTICES IF THEY ACCIDENTALLY HIT 'EDIT'
	if ( isset( $uvals['envoyconnect_user_meta'] ) ) {			
	
		// GO AHEAD AND PROCESS THE FULL SUBMISSION
		foreach ( $uvals['envoyconnect_user_meta'] as $key => $value ) {
			
			$envoyconnect_{$key} = envoyconnect_get_option( $key );
			if ( empty( $envoyconnect_{$key} ) )
				continue;
			
			// IF THIS IS A SECTION, SKIP IT
			if ( 'section' == $envoyconnect_{$key}['options']['field_type'] )
				continue;
			
			if ( is_array( $value ) ) {
				
				// FOR MULTI-TEXT LOOP THROUGH TO MAKE SURE NULL VALUES ARE REMOVED FROM PROFILE EDITS
				if ( 'multitext' == $envoyconnect_{$key}['options']['field_type'] ) {
					$new_value = array();
				...
				// FOR TAXONOMIES
				if ( 'taxonomy' == $envoyconnect_{$key}['options']['field_type'] ) {

					if ( isset( $uvals['envoyconnect_user_taxonomy_options'] ) )
						$data_handler = $uvals['envoyconnect_user_taxonomy_options'][$key];
					
					if ( 'append' == $data_handler || 'append_no_overwrite' == $data_handler ) {
						$pre_array = get_user_meta( $user_id, envoyconnect_key( $key, $envoyconnect_{$key}['source'] ), true );
						if ( is_array( $pre_array ) ) {
							...
							//update_user_meta( $user_id, envoyconnect_key( $key, $envoyconnect_{$key}['source'] ), $merged );
						} else {
							//update_user_meta( $user_id, envoyconnect_key( $key, $envoyconnect_{$key}['source'] ), $value );
						}
			...
			if ( 'wpr' == $envoyconnect_{$key}['source'] && 'url' == $key ) {
				...
				if ( false !== strpos( $data_handler, 'no_overwrite' ) ) {
					${$key} = get_user_meta( $user_id, envoyconnect_key( $key, $envoyconnect_{$key}['source'] ), true );
					if ( '' != ${$key} )
						continue;
				}
				
				if ( empty( $value ) ) {
					delete_user_meta( $user_id, envoyconnect_key( $key, $envoyconnect_{$key}['source'] ) );
				} else {
					update_user_meta( $user_id, envoyconnect_key( $key, $envoyconnect_{$key}['source'] ), $value );
				}
				
			}
			/*} elseif ( 'envoyconnect' != $envoyconnect_{$key}['source'] ) {
				...
				// CHECK TO MAKE SURE WE'RE ALLOWING OVERWRITES
				if ( isset( $_POST['no_overwrite'] ) ) {
					${$key} = get_user_meta( $user_id, 'envoyconnect_'.$key, true );
					if ( '' != ${$key} )
						continue;
				}
	
				if ( '' != $value || !empty( $value ) ) {
					update_user_meta( $user_id, 'envoyconnect_'.$key, $value );
				} else {
					delete_user_meta( $user_id, 'envoyconnect_'.$key );
				}
			}*/
	...
	if ( isset( $uvals['envoyconnect_user_taxonomy'] ) ) {
		foreach ( $uvals['envoyconnect_user_taxonomy'] as $key => $value ) {
	...
	*/
	...
	do_action( 'envoyconnect_update_user_metadata_ext', $user_id, $uvals );
 ...
 * @param $envoyconnect_cap str. The envoyconnect-specific capability
 * @param $plu
 * @param $param
 * @param $param
 * @return return type
...
function envoyconnect_hide_meta( $args = '' ) {
	...
	if ( '-bulk-edit' == $action || 'bulk-edit' == $action ) {
		...
		$allow = get_option( '_envoyconnect_bulk_edit_allow' );
		if ( false == $allow )
			$allow = array();
...
/*
function envoyconnect_get_field_status( $val, $key, $args ) {
...
function envoyconnect_get_field_break( $meta_key, $user_meta, $args = array(), $attribute ) {
	if ( empty( $args ) ) {
		return $meta_key;
		...
		foreach ( $args as $key => $val ) {
			if ( 'value' != $attribute ) {
				if ( array_walk_recursive( $user_meta, 'envoyconnect_get_field_status', $val ) )
					return $meta_key;
			} else {
...
function envoyconnect_get_user_metadata( $args = '' ) {
	...
	$envoyconnect_user_meta_arr = array();
	
	foreach ( $envoyconnect_user_meta as $key => $val ) {
	
		// IF IT'S IN A COLUMN
		if ( is_array( $val ) ) {
			foreach ( $val as $subkey => $subval )
				$envoyconnect_user_meta_arr = envoyconnect_get_user_metadata_val( $envoyconnect_user_meta_arr, $subval, $args );
				
		// IF IT'S FREE
		} else {
			$envoyconnect_user_meta_arr = envoyconnect_get_user_metadata_val( $envoyconnect_user_meta_arr, $val, $args );
		}
	...
	if ( !empty( $include ) ) {
		foreach ( $envoyconnect_user_meta_arr as $akey => $aval ) {
			if ( !in_array( $aval['options']['field_type'], $include ) )
				unset( $envoyconnect_user_meta_arr[$akey] );
	...
	return array_filter( $envoyconnect_user_meta_arr );
	
}


// LOCALLY FILTER THE ARRAY
function envoyconnect_get_user_metadata_val( $envoyconnect_user_meta_arr, $val, $args ) {
	
	// GRAB THE META DATA
	$user_meta = envoyconnect_get_option( $val );
	...
	if ( 'section' == $user_meta['options']['field_type'] ) {
		...
		if ( false != $args['section_break'] ) {
			$envoyconnect_user_meta_arr[] = envoyconnect_val( $user_meta, $args['return_val'] );
		} else {
			...
			if ( isset( $user_meta['options']['choices'] ) && !empty( $user_meta['options']['choices'] ) ) {
				foreach ( $user_meta['options']['choices'] as $section_key => $section_val ) {
					...
					// GRAB THE META DATA
					$section_meta = envoyconnect_get_option( $section_val );
					...
					if ( 'group' == $section_meta['options']['field_type'] ) {
						...
						if ( false != $args['group_break'] ) {
							$envoyconnect_user_meta_arr[] = envoyconnect_val( $section_meta, $args['return_val'] );
						} else {
							// LOOP THROUGH ITS CHILDREN
							foreach ( $section_meta['options']['choices'] as $group_key => $group_val ) {
								$envoyconnect_user_meta_arr[] = envoyconnect_val( envoyconnect_get_option( $group_val ), $args['return_val'] );
							}
						}
	...
	return $envoyconnect_user_meta_arr;
...
function envoyconnect_primary_marker( $meta, $user_id = '' ) {
	
	if ( isset( $meta['group_type'] ) && 'address' == $meta['group_type'] ) {
	
		$prim_test = get_user_meta( $user_id, 'envoyconnect_pp_primary', true );
		...
		?>
			<span class="envoyconnect-qualifier">
				<input type="radio" name="envoyconnect_user_meta[pp_primary]" class="envoyconnect-primary" value="<?php echo $key; ?>"<?php echo $p_check; ?> /> <span><?php _e( 'primary', 'envoyconnect' ); ?></span>
			</span>
		<?php 
	}
