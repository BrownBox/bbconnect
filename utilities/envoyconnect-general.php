...
function envoyconnect_random( $args = null ) {
...
function envoyconnect_admin_fixes() {
	//wp_tiny_mce( true );
	// FIX IE 8 ISSUES
	?>
	<!--[if lt IE 9]>
...
function envoyconnect_minified_admin() {
	...
	if ( is_admin() ) {
		...
		if ( $pagenow == 'post.php' || $pagenow == 'post-new.php' || 'users.php' == $pagenow ) {
	    	add_action( 'admin_head', 'envoyconnect_minify_admin' );
	    }
...
function envoyconnect_minify_admin() {
	global $post, $pagenow;
	...
	} else if ( 'users.php' == $pagenow && isset( $_GET['page'] ) && 'envoyconnect_modal_action' == $_GET['page'] ) {
		$minify = true;
	}
	...
	if ( $minify ) {
		//if ( isset( $_GET['uid'] ) || isset( $_GET['pid'] ) || isset( $_POST['envoyconnect_iframe'] ) ) {
		?>
 ...
 * @since 1.0.0
 */
function envoyconnect_query_vars( $qvars ) {
	$envoyconnect_okget = apply_filters( 'envoyconnect_okget', array( 'envoyconnect', 'pp_key', 'pp_ref', 'pp_action' ) );
	foreach ( $envoyconnect_okget as $okgets )
...
/**
 * Basic rewrite rule that says, if we have a subset of EnvoyConnect, apply our matching and set envoyconnect as a key.
 * If this is a top-level EnvoyConnect request, let the parse request take over and set envoyconnect as the value.
 *
 * @since 1.0.0
 */
function envoyconnect_rewrite_rules( $wp_rewrite ) {
	$new_rules = array(
						'envoyconnect/([^/]*)?$' => 'index.php?envoyconnect=$matches[1]',
						'envoyconnect/([^/]*)/([^/]*)/([^/]*)?$' => 'index.php?envoyconnect=$matches[1]&pp_key=$matches[2]&pp_ref=$matches[3]',
	);
	$wp_rewrite->rules = $new_rules + $wp_rewrite->rules;
}

function envoyconnect_flush_rules(){

	// GET THE STORED RULES
	$rules = get_option( 'rewrite_rules' );

	if ( !isset( $rules['envoyconnect/([^/]*)/([^/]*)/([^/]*)?$'] ) || !isset( $rules['envoyconnect/([^/]*)?$'] ) ) {
		global $wp_rewrite;
 ...
 * @since 1.0.0
 */
function envoyconnect_parse_request( $wp ) {

	// DO WE HAVE PAUPRESS AS AN EXPLICIT VALUE?
	$pp_top = in_array_r( 'envoyconnect', $wp->query_vars, true );

	// DO WE HAVE PAUPRESS AS AN INVISIBLE REFERENCE?
	if ( isset( $wp->query_vars['envoyconnectref'] ) ) {
		if ( 'true' == get_option( 'envoyconnectpanels_embed' ) && false == $pp_top ) {
			$pp_top = 'pagename';
			$wp->query_vars['pagename'] = 'envoyconnect';
		}
	}
    ...
    if ( array_key_exists( 'envoyconnect', $wp->query_vars ) ) {
		switch( $wp->query_vars['envoyconnect'] ) {

			// EXTEND THE HOOK SYSTEM
			default :
				do_action( 'envoyconnect_parse_switch', $wp );
			break;
	...
	// PROCESS EMBEDS AND REDIRECTS
		...
		//if ( 'pagename' == $pp_top || 'name' == $pp_top ) {
			...
			add_filter( 'wp_title', 'envoyconnect_page_title' );
			do_action( 'envoyconnect_system_page', $wp );
		//}
	}
...
function envoyconnect_page_title() {
	global $wp_query;
	return sprintf( __( '%1$s', 'envoyconnect' ), ucfirst( $wp_query->query_vars['rel'] ) );
}

function envoyconnect_error_log( $args = null ) {
	...
	$error_log = get_option( '_envoyconnect_error_log' );
	if ( !is_array( $error_log ) )
		$error_log = array();

	array_push( $error_log, $args );

	return update_option( '_envoyconnect_error_log', $error_log );

}

function envoyconnect_hello_dolly( $id, $args = null ) {
