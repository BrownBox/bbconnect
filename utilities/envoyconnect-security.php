...
function envoyconnect_scrub( $callback, $data ) {
    if ( is_array( $data ) ) {
		$new = array();
		foreach ( $data as $key => $val ) {
			if ( is_array( $val ) ) {
				$new[$key] = envoyconnect_scrub( $callback, $val );
			} else {
...
function envoyconnect_restrict_redirect(){
    ...
    if ( is_admin() ) {
    	...
    	if ( 'true' === get_option( '_envoyconnect_access' ) ) {
    	...
    	if ( $pagenow == 'profile.php' || $pagenow == 'user-edit.php' ) {
        	wp_redirect( admin_url( 'admin.php?page=envoyconnect_edit_user_profile&user_id=' . $_GET['user_id'] ) );
        	die();
        }
        
        // IF THEY'RE TRYING TO ACCESS THE ORIGINAL CREATE PROFILE PAGE, REDIRECT THEM
        if ( $pagenow == 'user-new.php' ) {
        	wp_redirect( admin_url( 'admin.php?page=envoyconnect_new_user' ) );
        	die();
        }
        
        // IF THEY'RE TRYING TO ACCESS THE ORIGINAL ALL USERS PAGE, REDIRECT THEM
        if ( $pagenow == 'users.php' && !isset( $_GET['page'] ) ) {
        	if ( 'true' != get_option( '_envoyconnect_compatability_mode' ) ) {
	        	wp_redirect( admin_url( 'users.php?page=envoyconnect_reports' ) );
	        	die();
	        }
        }
 ...
 */
function envoyconnect_sanitize( $str ) {
